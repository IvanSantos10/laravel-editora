...
class PermissionReader
{
    ...
    public function getPermission($controllerClass, $action = null)
    {
        ...
        /** @var Controller $controlllerAnnotation */
        $controlllerAnnotation = $this->reader->getClassAnnotation($rc, Controller::class);
        $permissions = [];
        if ($controlllerAnnotation) {
            $permission = [
                'name' => $controlllerAnnotation->name,
                'description' => $controlllerAnnotation->description,
            ];
    ...
    private function getControllers()
    {
        ...
        $config = include  __DIR__ . '/../Http/Controllers';
        $dir = array_merge($dirs, $config['acl']['controllers_annotations']);
        $files = [];
        foreach ($dirs as $dir) {
